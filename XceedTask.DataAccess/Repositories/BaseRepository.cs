...
public class BaseRepository<T>(ApplicationContext context) : IBaseRepository<T>
    where T : class
{
    protected ApplicationContext Context = context;

    public IEnumerable<T> GetAll(Func<IQueryable<T>, IIncludableQueryable<T, object>> include = null, Func<IQueryable<T>, IOrderedQueryable<T>> orderBy = null, bool isNoTracking = true)
    {
    ...
    public async Task<IEnumerable<T>> GetAllAsync(Func<IQueryable<T>, IIncludableQueryable<T, object>> include = null, Func<IQueryable<T>, IOrderedQueryable<T>> orderBy = null, bool isNoTracking = true)
    {
        IQueryable<T> query = Context.Set<T>();
        if (include != null)
            query = include(query);
    ...
    public async Task<IEnumerable<T>> AddBulkInsert(IEnumerable<T> entities)
    {
    ...
    public int DeleteAll(Expression<Func<T, bool>> criteria)
    {
        IQueryable<T> query = Context.Set<T>();
        query = query.Where(criteria);
    ...
    public void DeAttachRange(IEnumerable<T> entities)
    {
